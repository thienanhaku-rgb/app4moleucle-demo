...
const NAV_ITEMS = [
  ...
  { name: 'Simulation Lab', href: '/simulation', icon: Microscope },
...
export function Sidebar({ history = [], onHistoryClick }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'flex-col border-r border-border bg-card/50 backdrop-blur-xl z-20 h-full shadow-sm relative hidden md:flex transition-all duration-300',
    ...
    >
          ...
          return (
            <Link key={item.href} href={item.href}>
              ...
              >
                ...
                <span
                  className={cn(
                    'relative z-10 transition-transform group-hover:scale-110 duration-200',
                    isActive && 'scale-110'
                  )}
                >
          ...
          <div className="space-y-1">
            {history.slice(0, 10).map((record) => (
              <div
                ...
                className="px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-accent/50 cursor-pointer truncate transition-all"
              >
      ...
      <div className="p-4 border-t border-border/50">
        <Button
          variant="ghost"
          className="w-full text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-all justify-between"

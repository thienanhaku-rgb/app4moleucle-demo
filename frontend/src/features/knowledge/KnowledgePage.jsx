...
import { 
  Send, Bot, User, BookOpen, Sparkles, MoreHorizontal
} from "lucide-react";
...
export default function KnowledgePage() {
  ...
  const handleSend = async () => {
      ...
      try {
          const res = await axios.post(getApiUrl("/api/knowledge/chat"), { query: userMsg.content });
          const botMsg = { 
              role: 'assistant', 
        ...
        <div className="flex h-full flex-col md:flex-row overflow-hidden">
            ...
            <div className="w-full md:w-80 border-r border-border bg-card/30 hidden md:flex flex-col">
                ...
                <div className="p-4 space-y-2">
                    ...
                    {["Aspirin Synthesis", "Ethanol Properties", "Caffeine Structure", "Novel Inhibitors"].map(topic => (
                        <div key={topic} className="p-3 rounded-xl bg-card border border-border/50 hover:border-primary/50 cursor-pointer transition-all text-sm font-medium text-foreground/80 hover:text-primary">
                            {topic}
                        </div>
                    ))}
                </div>
